...
export const NightDetails = dict(
    [Role.WEREWOLF, {
        ...
        should_play: function(game) {
            ...
            return game.players.filter(x => x.faction == Faction.WEREWOLVES).length > 0;
        }
    }],
    [Role.HEALER, {
    ...
    [Role.JESTER, {
        ...
        timer: 10000,
        should_play: function(game) {
            return game.players.filter(x => x.role == Role.JESTER && x.haunting).length > 0;
        }
    }],
    [Role.VETERAN, {
    ...
    [Role.SPOOKY_DOLL, {
        summon_message: "Player with the spooky doll, wake up. Pick a player to pass the doll to.",
