...
export class Player {
  ...
  isActive__doll(game) {
    return (
      game.nightPlayOrder[game.nightIndex] == Role.SPOOKY_DOLL &&
      !this.dead &&
      this.holds_doll &&
      this.doll_giveto == null
    );
  }
  ...
  calculateKill(game) {
    var attack = this.attackers.length
      ? maxOf(this.attackers, (x) => x[1])
      : null;
    ...
    if (attack) {
      if (!defense || attack[1] > defense[1]) {
        if (this.holds_doll) {
          ...
          if (this.holds_doll) {
            game.clearDoll();
            var creepyGirl = game.players.filter(
              (x) => x.role == Role.CREEPY_GIRL && !x.dead
            );
            if (creepyGirl.length) {
              for (var girl of creepyGirl) {
                girl.setRole(Role.DEATH_WITCH);
                girl.sendMessage(
                  "Your doll has been taken to the grave, and you are now a Death Witch!"
                );
              }
          ...
          var callouts = [
            "Tonight, we found " + this.name + ", dead in their home.",
          ];
          for (var a in this.attackers) {
            callouts.push(
              (a == 0 ? "They were apparently " : "They were also ") +
        ...
        var callouts = [
          this.name + " was apparently attacked tonight, but they survived",
        ];
        return callouts;
      }
    }
  ...
  isVictorious(winning_faction) {
    return (
      winning_faction == this.faction || ~winning_faction.indexOf(this.faction)
    );
  }
